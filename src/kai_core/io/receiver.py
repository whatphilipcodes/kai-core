...
class Receiver:
    ...
    def start(self) -> None:
        ...
        if self._callback is None:
            logger.warning("Starting receiver without a registered callback.")
            
        self._running = True
        self._thread = threading.Thread(target=self._listen, daemon=True)
        self._thread.start()
        logger.info(f"ZMQ Receiver listening on {self.addr_incoming}")
        ...
        while self._running:
            ...
            if self.socket in events and events[self.socket] == zmq.POLLIN:
                try:
                    ...
                    logger.debug(f"Message Received successfully: {message_bytes}")
